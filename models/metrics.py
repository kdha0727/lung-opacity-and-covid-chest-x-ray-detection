...
@torch.no_grad()
def classification_confusion_matrix(model, data, device=None):
    ...
    for x, y in data:
        if device is not None:
            x = x.to(device)
        pred.append(model(x).argmax(dim=1, keepdim=True).cpu())
        true.append(y)
    true = torch.cat(true)
    pred = torch.cat(pred)
...
__all__ = ['classification_confusion_matrix', 'draw_confusion_matrix']
